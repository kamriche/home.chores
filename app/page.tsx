    ...
    <div id="hearth-demo" data-theme={dark ? "dark" : "light"}>
      <div className="hc-shell">
        <aside className="hc-side">
          <div className="hc-brand"><span className="hc-logo"><House size={18} /></span>hearth<span style={{ color: "var(--hc-accent)" }}>.</span></div>
          <div className="hc-house-wrap">
            <button className="hc-house" type="button" aria-expanded={houseMenu} onClick={() => setHouseMenu(!houseMenu)}>
              <span>
                ...
                <span className="hc-label">{space.people.length} people · {space.rooms.length} rooms · {space.floors} floor{space.floors === 1 ? "" : "s"}</span>
          ...
          <div className="hc-side-bottom"><div className="hc-label">A little together, every day.</div><p style={{ fontSize: 12, marginTop: 8 }}>A happier home starts with a shared load.</p></div>
        </aside>
        ...
        <main className="hc-main">
          ...
          {editor === "space" && <SpaceForm onCancel={() => setEditor(null)} onSubmit={(name, type) => {
            ...
            setSpaces((current) => [...current, { id, name, type, floors: 1, responsibleId: personId, people: [{ id: personId, name: "Jamie", role: "Family member" }], rooms: [{ id: personId + 1, name: "Kitchen", type: "Kitchen", floor: 1 }], chores: [] }]);
...
function Heading({ title, subtitle, action }: { title: string; subtitle: string; action?: React.ReactNode }) {
  return <div className="hc-heading"><div><h1>{title}</h1><p>{subtitle}</p></div>{action}</div>;
}
...
function ChoresView({ space, currentUserId, mine, setMine, editor, setEditor, setChore, addChore, notify }: {
  ...
  return <>
    <Heading title="A home we care for, together." subtitle="Small tasks. Shared responsibility." action={<button className="hc-button hc-primary" onClick={() => setEditor("chore")}><Plus size={16} /> Add chore</button>} />
