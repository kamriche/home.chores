...
function Heading({ subtitle, action }: { subtitle: string; action?: React.ReactNode }) {
  return <div className="hc-heading"><div><p>{subtitle}</p></div>{action}</div>;
}
...
function ChoresView({ space, currentUserId, mine, setMine, editor, setEditor, setChore, addChore, notify }: {
  ...
  return <>
    <Heading subtitle="Small tasks. Shared responsibility." action={<button className="hc-button hc-primary" onClick={() => setEditor("chore")}><Plus size={16} /> Add chore</button>} />
